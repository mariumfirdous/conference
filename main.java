...
public class main{
     ...
     public static void main(String []args){
        ...
        for(int i=0;i<session.length;i++){
            ...
            totTime = time + sessionDuration;
            
            // Round off values
            totTime = Math.round(totTime * 100.00f) / 100.00f;
            time = Math.round(time * 100.00f) / 100.00f;
            
            float currMinute = 0;
            if (totTime < 10.00f) {
                currMinute = totTime % 1;
            } else {
                currMinute = totTime % 1;
            }
            ...
            if(time<12){
                System.out.println(time+"AM"+" "+session[i].topic);
                    
                    
            } else if(time >= 12) {
                System.out.println(time+"PM"+" "+session[i].topic);
            }
